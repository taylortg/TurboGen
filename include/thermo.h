...
class ThermoProps {
   public:
    ...
    ThermoProps(const std::string& fluid_name = "AIR") : state(CoolProp::AbstractState::factory("HEOS", fluid_name)) {}

    ThermoProps(const ThermoProps& other)
        : props(other.props),
          state(other.state ? CoolProp::AbstractState::factory("HEOS", other.state->fluid_names()) : nullptr) {}
